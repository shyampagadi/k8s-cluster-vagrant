...
output "alarm_count" {
  ...
  value       = length(aws_cloudwatch_metric_alarm.cpu_utilization) + length(aws_cloudwatch_metric_alarm.disk_utilization) + length(aws_cloudwatch_metric_alarm.memory_utilization)
}

# Troubleshooting Configuration
output "troubleshooting_configuration" {
  description = "Troubleshooting configuration summary"
  value = {
    troubleshooting_enabled = var.enable_troubleshooting
...
output "troubleshooting_thresholds" {
  description = "Troubleshooting thresholds"
  value = {
...
output "troubleshooting_features" {
  description = "Troubleshooting features status"
  value = {
...
output "troubleshooting_status" {
  description = "Status of troubleshooting implementation"
  value = {
    troubleshooting_enabled = var.enable_troubleshooting
    debugging_enabled = var.enable_debugging
    validation_enabled = var.enable_validation
    ...
    logging_enabled = var.enable_logging
    health_checks_enabled = var.enable_health_checks
    alarm_count = length(aws_cloudwatch_metric_alarm.cpu_utilization) + length(aws_cloudwatch_metric_alarm.disk_utilization) + length(aws_cloudwatch_metric_alarm.memory_utilization)
  }
}
...
output "troubleshooting_endpoints" {
  description = "Troubleshooting-related endpoints and information"
  value = {
    ...
    memory_alarms = aws_cloudwatch_metric_alarm.memory_utilization[*].alarm_name
  }
}
...
output "estimated_monthly_cost" {
  ...
  value = {
    ...
    cloudwatch_alarms = var.enable_cloudwatch_alarms ? 5 : 0 # Approximate cost for CloudWatch alarms
    total = (length(aws_instance.web) * 10) + (var.create_database ? 25 : 0) + 5 + (var.enable_monitoring ? 10 : 0) + 20 + (var.enable_performance_insights ? 5 : 0) + (var.enable_cloudwatch_alarms ? 5 : 0)
  }
}
